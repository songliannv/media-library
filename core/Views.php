...
HTML;return str_replace('{CSS_PLACEHOLDER}',self::css(),$j2);}public static function renderItemPage(array $a){$b=(int)((isset($a['id'])?$a['id']:0));$c=(isset($a['type'])?$a['type']:'');$d=(isset($a['source'])?$a['source']:'');$e=self::esc((isset($a['title'])?$a['title']:'未命名'));$f=self::esc((isset($a['original_title'])?$a['original_title']:''));$g=!empty($a['year'])?(int)$a['year']:'';$h=($a['rating']!==null&&$a['rating']!=='')?number_format((float)$a['rating'],1):'—';$i=self::typeLabel($c);$j=(isset($a['poster'])?$a['poster']:'');$k=(isset($a['backdrop'])?$a['backdrop']:'');$l=(int)((isset($a['clicks'])?$a['clicks']:0));$m=[];foreach(json_decode((isset($a['genres'])?$a['genres']:'[]'),true)?:[]as $n){if($n)$m[]=self::esc($n);}$o='';if($m){foreach($m as $n){$o.='<span class="t">'.$n.'</span>';}}$p=$k?'style="background-image:url('.self::esc($k).')"':'';$q=$j?'<img src="'.self::esc($j).'" alt="'.$e.'" loading="lazy">':'<div class="np">无封面</div>';$r=self::esc((isset($a['overview'])?$a['overview']:''));$s=$r?'<div class="block"><h2>简介</h2><div class="ov">'.$r.'</div></div>':'';$t=self::downloadBlock($a);$u=self::extraBlock($a,$d,$c);$v=Site::headMeta('item',array('id'=>$b,'title'=>isset($a['title'])?(string)$a['title']:'','type_label'=>$i,'year'=>($g!==''?(string)$g:''),'overview'=>isset($a['overview'])?(string)$a['overview']:'','poster'=>$j,));$w=self::topbarHtml('<a class="admin" href="/">← 返回主页</a>');$x=self::footerHtml();$y=self::bodyClass();$z=(bool)Config::sub('admin','show_link',1);$a1='';if($z){$a1='<div class="adm-btn"><a href="'.self::esc(self::adminUrl()).'">'.(Site::flag(Config::sub('admin','show_icon',1))?'🛡 ':'').'管理后台</a></div>';}$b1=<<<HTML
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
