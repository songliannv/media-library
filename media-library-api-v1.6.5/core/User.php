...
class User
{
  ...
  KEY `idx_title` (`title`(191))
     ...
     */
    public static function register($username, $pass, $pass2, $email, $captcha, $honeypot = '')
    {
        $c = self::cfg();
        if (!$c['open']) {
        ...
        $err = self::validUsername($username);
        if ($err !== '') { return self::fail($err); }
        $err = self::validPassword($pass, $username);
        if ($err !== '') { return self::fail($err); }
        ...
        if (self::findByUsername($username) !== null) {
            return self::fail('这个用户名已经被使用了，换一个吧。');
        }
    ...
    public static function login($username, $pass)
    {
        ...
        if ($username === '' || (string) $pass === '') {
            return self::fail('请填写用户名和密码。');
        }

        $row = self::findByUsername($username);
        /* 用户名不存在也走一次散列计算，避免通过响应快慢判断账号是否存在 */
        if ($row === null) {
            password_verify((string) $pass, '$2y$10$usesomesillystringforsalt0000000000000000000000000000000000');
            return self::fail('用户名或密码不对。');
        }
