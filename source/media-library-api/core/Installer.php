...
if (!function_exists('mlinst_config_content')) {
     ...
     * @param string $adminToken  已废弃（v1.7.0 后台改用账号+密码）；保留形参以兼容旧调用
    ...
    function mlinst_config_content($db, $tmdbKey, $rawgKey, $adminToken, $adapters)
    {
        ...
        $s .= $t . "    'proxy_key'   => '',\n";
        $s .= $t . "],\n\n";
        /* ★ v1.7.0：后台改用「账号 + 密码」登录，config.php 里不再有 admin.token。
           这行注释保留，是为了让看过旧版文档的人一眼明白改了什么。 */
        $s .= $t . "// 后台目录与显示（★ v1.7.0 起后台用账号+密码登录，不再有 admin.token）\n";
        $s .= $t . "'admin' => [\n";
        $s .= $t . "    'dir'       => 'admin',   // 后台目录名（必须以 admin 结尾）\n";
        $s .= $t . "    'show_link' => 1,           // 1=主页显示管理后台按钮；0=隐藏\n";
        $s .= $t . "    'show_icon' => 1,           // 1=按钮带图标；0=纯文字\n";
        $s .= $t . "],\n\n";
...
if (!function_exists('mlinst_normalize')) {
    ...
    function mlinst_normalize($in)
    {
        ...
        return array(
            ...
            'admin_pass' => isset($in['admin_pass']) ? (string) $in['admin_pass'] : '',
            'admin_pass2' => isset($in['admin_pass2']) ? (string) $in['admin_pass2'] : '',
            'tmdb'     => $g('tmdb'),
            'rawg'     => $g('rawg'),
...
if (!function_exists('mlinst_validate')) {
    ...
    function mlinst_validate($in)
    {
        ...
        /* 管理员账号：安装时必须创建，用于登录后台。v1.8.3 起推荐直接用邮箱 */
        $au = $in['admin_user'];
        if ($au === '') {
            return '请填写管理员账号（推荐用邮箱，登录后台用）。';
        }
        if (strpos($au, '@') !== false) {
            /* 带 @ 的按邮箱校验：管理员账号即邮箱，找回密码邮件也发这里 */
            if (strlen($au) > 128 || !filter_var($au, FILTER_VALIDATE_EMAIL)) {
                return '管理员账号邮箱格式不正确，请检查（示例：admin@example.com）。';
            }
        } elseif (!preg_match('/^[A-Za-z0-9_.-]{3,32}$/', $au)) {
            return '管理员账号只能包含字母、数字、下划线、点、短横线，长度 3~32 位；或直接填邮箱。';
        }
        if (strlen($in['admin_pass']) < 6) {
            return '管理员密码至少 6 位。';
        }
...
if (!function_exists('mlinst_install')) {
     ...
     * @return array ok/error/steps/admin_user/mysql/files/config_file/lock_file
     */
    function mlinst_install($root, $raw)
    {
        ...
        $steps[] = array('label' => '创建数据表', 'ok' => true,
            'detail' => 'media_items / sync_log / link_checks / app_settings / app_users / app_requests / app_api_tokens / app_api_logs');

        /* ---- 4.5) 创建第一个账号 = 管理员账号（用于登录后台） ---- */
        try {
            $hash = password_hash($in['admin_pass'], PASSWORD_DEFAULT);
            if ($hash === false || $hash === null) {
                /* 极端环境（无 bcrypt）兜底：用 sha256 + 固定盐，仍然可用 */
            ...
            $stm->execute(array(
                ':u' => $in['admin_user'],
                /* 管理员身份靠「第一个账号」规则（User::isAdmin 的 MIN(id) 通道）；
                   账号是邮箱时 email 列写真邮箱 —— 忘记密码时重置邮件才有地方发 */
                ':e' => (strpos($in['admin_user'], '@') !== false) ? $in['admin_user'] : 'ml-admin@local',
                ':p' => $hash,
                ':c' => $now,
            ));
        } catch (Exception $e) {
            $res['error'] = '创建管理员账号失败：' . $e->getMessage();
            $steps[] = array('label' => '创建管理员账号', 'ok' => false, 'detail' => $res['error']);
            $res['steps'] = $steps;
            return $res;
        }
        $steps[] = array('label' => '创建管理员账号', 'ok' => true,
            'detail' => '账号「' . $in['admin_user'] . '」已创建（这是后台管理员，请记牢密码）');
        ...
        $cfgText = mlinst_config_content($dbCfg, $in['tmdb'], $in['rawg'], '', $in['adapters']);
        ...
        $steps[] = array('label' => '生成 config/config.php', 'ok' => true, 'detail' => '已写入数据库配置');
