 ...
 * ==================================================================
 * 设计目标：让「TMDB Key / RAWG Key / 启用数据源 / 网盘类型」
...
class Settings
{
    ...
    {
        $base = array(
            /* ★ v1.7.0：admin_token 已移除 —— 后台改为账号 + 密码登录，
               不再有「后台令牌」这项设置。外部调用者的令牌走 app_api_tokens 表（「API 调用」页）。 */
            'tmdb_api_key', 'rawg_api_key', 'adapters', 'pan_types',
    ...
    /**
     * 邮件服务（SMTP）：后台「系统设置 → 邮件服务」，用于「找回密码 / 重置密码」发信。
     * 落到 $cfg['mail'] 段，消费方是 core/Mail.php。
     */
    public static function mailMap()
    {
        return array(
            'mail_enable'    => 'enable',
            'mail_host'      => 'host',
    ...
    public static function schema()
    {
        $out = array(
            ...
            'sync_order' => array(
                ...
                'label' => '按什么口径取内容',
                'tip'   => '最热 = TMDB trending / RAWG -added / 红果短剧当日热门；最多人看 = TMDB popular 榜 / RAWG -rating / 红果短剧排行榜。',
            ),
            'sync_min_votes' => array(
                'type' => 'int', 'group' => 'sync', 'min' => 0, 'max' => 1000000, 'unit' => '票',
                'label' => '投票数低于此值的条目不要',
                'tip'   => '这是「最多人看」的量化门槛：TMDB 用 vote_count、RAWG 用 rating、红果短剧用播放量。0 = 不限。填 100 可过滤掉冷门条目。',
            ),
            'sync_dedupe' => array(
                'type' => 'select', 'group' => 'sync',
            ...
            'sync_skip_same_title' => array(
                ...
                'tip'   => '例如同一部片在 TMDB 与红果短剧都有：开启后只保留先采集到的那条，站内不会出现两遍。年份允许差 1 年。',
            ),
            'sync_interval' => array(
            ...
            /* ================= 系统设置 · 邮件服务（SMTP） ================= */
            'mail_enable' => array(
                'type' => 'select', 'group' => 'mail',
                'options' => array('1' => '启用', '0' => '停用'),
                'label' => '启用邮件服务',
                'tip'   => '用于「找回密码 / 重置密码」发信。启用后请填好下面的 SMTP 信息，并用「发送测试邮件」验证。',
            ),
            'mail_host' => array(
                'type' => 'text', 'group' => 'mail',
                'label' => 'SMTP 服务器地址',
                'tip'   => '例：smtp.qq.com（QQ 邮箱）、smtp.163.com（网易）、smtp.mxhichina.com（阿里云）。',
            ),
            'mail_port' => array(
                'type' => 'int', 'group' => 'mail', 'min' => 1, 'max' => 65535, 'unit' => '端口',
                'label' => '端口',
                'tip'   => 'SSL 一般 465，STARTTLS 一般 587，不加密 25。要和下面的「加密方式」配套。',
            ),
            'mail_secure' => array(
                'type' => 'select', 'group' => 'mail',
                'options' => array('ssl' => 'SSL（465）', 'tls' => 'STARTTLS（587）', 'none' => '不加密（25）'),
                'label' => '加密方式',
                'tip'   => '绝大多数邮箱用 SSL + 465。选 STARTTLS 时端口填 587。',
            ),
            'mail_user' => array(
                'type' => 'text', 'group' => 'mail',
                'label' => 'SMTP 账号',
                'tip'   => '通常是完整邮箱地址，例：you@qq.com。',
            ),
            'mail_pass' => array(
                'type' => 'text', 'group' => 'mail',
                'label' => 'SMTP 密码 / 授权码',
                'tip'   => 'QQ / 163 等要填「授权码」（在邮箱设置里开启 SMTP 服务后生成），不是登录密码。',
            ),
            'mail_from' => array(
                'type' => 'text', 'group' => 'mail',
                'label' => '发件人邮箱',
                'tip'   => '必须与上面的 SMTP 账号一致，否则多数邮箱会拒发。例：you@qq.com。',
            ),
            'mail_from_name' => array(
                'type' => 'text', 'group' => 'mail',
                'label' => '发件人名称',
                'tip'   => '收件人看到的发件人显示名，例：某某资料库。留空则用发件人邮箱。',
            ),
            'mail_timeout' => array(
                'type' => 'int', 'group' => 'mail', 'min' => 5, 'max' => 60, 'unit' => '秒',
                'label' => '发信超时',
                'tip'   => '连不上 SMTP 服务器时的等待上限。一般 15 秒即可。',
            ),
        );
  ...
  PRIMARY KEY (`k`)
     ...
     * ★ v1.8.7：也接受数组直接传入 —— 前端复选框/多选是 JSON 数组，
     * 老代码只做 (string) 强转，数组会被转成字面量 "Array" 而全部丢失。
     */
    public static function strToList($s)
    {
        if (is_array($s)) { $s = self::listToStr($s); }
