<?php
/**
 * 媒资资料库 · 网页安装器（install.php）
 * ==================================================================
 * 行为（v1.6.3 起）：
 *   ① 已安装 → 直接 404
 *      判定标准就是 core/InstallGuard.php 的 ml_install_state()：
 *      存在 config/install.lock，或 config/config.php 的 db 段已填真值。
 *      也就是说，只要站点装好了，本文件对任何人（包括你）都"不存在"，
 *      没人能借它重装站点或读到服务器信息。
 *   ② 未安装 → 显示安装向导
 *      环境自检 → 填数据库与站点信息 → 一键安装（建表 + 生成 config.php +
 *      写 install.lock）→ 成功后本页面立即失效。
 *
 * 安全措施：
 *   · 只接受 POST 执行安装，且必须带会话 CSRF 令牌（恒时比较）
 *   · 蜜罐字段（真人看不见的那个输入框被填了就丢弃）
 *   · 同一会话失败 20 次后要求等待 10 分钟
 *   · 安装信息备忘写在 config/install-info.txt（该目录公网 404），不再裸放站点根
 *
 * 兼容 PHP 5.6 ~ 8.2。
 */
...
require_once $ROOT . '/core/Guard.php';
require_once $ROOT . '/core/InstallGuard.php';
require_once $ROOT . '/core/Installer.php';

/* ---------------------------------------------------------------- */
/* ① 已安装：与文件不存在完全一样                                      */
/* ---------------------------------------------------------------- */
$MLST = ml_install_state($ROOT);
if (!empty($MLST['installed'])) {
    ml_guard_deny();       // 输出 404 页面并 exit
}
...
if (empty($_SESSION['mlinst_csrf'])) {
    $_SESSION['mlinst_csrf'] = mlinst_rand_token();
}
$CSRF = (string) $_SESSION['mlinst_csrf'];
...
$hostHdr = isset($_SERVER['HTTP_HOST']) ? (string) $_SERVER['HTTP_HOST'] : '';
$DEF_URL = $hostHdr !== '' ? ($scheme . '://' . $hostHdr) : '';

if (isset($_POST['do']) && $_POST['do'] === 'install') {
    $FORM    = $_POST;
    $tries   = isset($_SESSION['mlinst_tries']) ? (int) $_SESSION['mlinst_tries'] : 0;
    $lastTry = isset($_SESSION['mlinst_last']) ? (int) $_SESSION['mlinst_last'] : 0;

    if ($tries >= 20 && (time() - $lastTry) < 600) {
        $ERR = '尝试次数过多，请等 10 分钟后再试。';
    } elseif (!isset($_POST['csrf']) || !mlinst_eq($_POST['csrf'], $CSRF)) {
        $ERR = '页面已过期（安全令牌不匹配），请重新填写提交一次。';
    } elseif (isset($_POST['website']) && trim((string) $_POST['website']) !== '') {
        $ERR = '检测到异常提交，已忽略。';
    } elseif (!$ENV_OK) {
        $ERR = '环境自检未通过，请先按下方红色提示处理后重试。';
    } else {
        $_SESSION['mlinst_tries'] = $tries + 1;
        $_SESSION['mlinst_last']  = time();

        $RESULT = mlinst_install($ROOT, $_POST);
        if (empty($RESULT['ok'])) {
            $ERR = (string) $RESULT['error'];
        } else {
            $_SESSION['mlinst_csrf']  = mlinst_rand_token();
            $_SESSION['mlinst_tries'] = 0;
        }
    }
}

$DONE = ($RESULT !== null && !empty($RESULT['ok']));

/* 表单回填值 */
function ml_ins_val($FORM, $k, $d = '')
{
    return isset($FORM[$k]) && is_scalar($FORM[$k]) ? (string) $FORM[$k] : $d;
}
$V_HOST  = ml_ins_val($FORM, 'host', '127.0.0.1');
$V_PORT  = ml_ins_val($FORM, 'port', '3306');
$V_DB    = ml_ins_val($FORM, 'dbname', '');
$V_USER  = ml_ins_val($FORM, 'user', '');
$V_PASS  = isset($FORM['pass']) ? (string) $FORM['pass'] : '';
$V_TOKEN = ml_ins_val($FORM, 'token', '');
$V_NAME  = ml_ins_val($FORM, 'site_name', '媒资资料库');
$V_URL   = ml_ins_val($FORM, 'site_url', $DEF_URL);
$V_TMDB  = ml_ins_val($FORM, 'tmdb', '');
$V_RAWG  = ml_ins_val($FORM, 'rawg', '');
$V_ADS   = isset($FORM['adapters']) && is_array($FORM['adapters']) ? $FORM['adapters'] : array('tmdb', 'rawg', 'bangumi');
function ml_ins_ck($V_ADS, $k)
{
    return in_array($k, $V_ADS, true) ? ' checked' : '';
}

$BASE_URL = $DEF_URL !== '' ? $DEF_URL : 'https://你的域名';
?><!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
...
.checks label{display:flex;align-items:center;gap:7px;margin:0;color:var(--txt);font-size:13px;cursor:pointer}
.checks input{width:16px;height:16px;accent-color:var(--brand)}
...
<div class="wrap">
  ...
  <div class="hero">
    ...
    <h1>媒资资料库 · <?php echo $DONE ? '安装完成' : '网页安装向导'; ?></h1>
    <p><?php echo $DONE
        ? '数据库已就绪、配置文件已生成。下面的令牌请务必记下。'
        : '跟着下面三步走：环境自检 → 填数据库信息 → 点一下安装。装完后本页面会自动失效。'; ?></p>
  </div>
...
<?php if ($DONE): /* ==================== 安装成功页 ==================== */ ?>
  ...
  <div class="card">
    ...
    <div class="okbox" style="margin-top:16px">
      <b>后台管理令牌（请立刻抄下来）</b>
      <div class="tk"><?php echo ml_ins_h($RESULT['token']); ?></div>
      <div class="muted" style="font-size:12px">后台登录用它（请求头 <code>X-Admin-Token</code> 或地址栏 <code>?token=</code>）。忘了可以在 <code>config/config.php</code> 的 <code>admin.token</code> 里找。</div>
    </div>
  </div>
...
location ~* ^/(setup|upgrade|update) { return 404; }</pre>
      ...
      <li style="margin:8px 0"><b>2. 打开后台</b>：<code><?php echo ml_ins_h($BASE_URL); ?>/admin</code>，粘贴上面的令牌进入。
        到「站点设置」填站点名称、网址、SEO，到「系统设置」填 TMDB / RAWG Key。</li>
      <li style="margin:8px 0"><b>3. 跑一次自检</b>：<code><?php echo ml_ins_h($BASE_URL); ?>/check.php?token=<?php echo ml_ins_h($RESULT['token']); ?></code>
        —— 11 组检查全绿就说明环境、权限、路由、安全防护都到位了。</li>
      ...
      <li style="margin:8px 0"><b>5. 清理</b>：<code>config/install-info.txt</code>（含令牌，记下后删掉）；
    ...
    <div class="acts">
      ...
      <a class="btn ghost" href="<?php echo ml_ins_h($BASE_URL); ?>/check.php?token=<?php echo ml_ins_h($RESULT['token']); ?>">打开环境自检</a>
    </div>
  </div>
...
<?php else: /* ==================== 安装表单页 ==================== */ ?>
  ...
  <input class="hp" type="text" name="website" value="" tabindex="-1" autocomplete="off">
  <input type="hidden" name="site_url_def" value="<?php echo ml_ins_h($DEF_URL); ?>">

  <div class="card">
    <h2><span class="n">2</span>数据库信息</h2>
    <div class="sub">先把库建好：宝塔 → 数据库 → 添加数据库；这里填的账号密码要和它一致。</div>
    <div class="grid">
      <div>
        <label>数据库地址</label>
      ...
      <div class="full">
        ...
        <div class="hint">如果是 MySQL 8 + PHP 5.6，请在宝塔里把该用户密码改成 mysql_native_password 认证</div>
      </div>
    </div>
  </div>

  <div class="card">
    <h2><span class="n">3</span>站点与密钥</h2>
    <div class="sub">这些装完也能在后台改，现在填好可以少跑一趟。</div>
    <div class="grid">
      <div>
        <label>站点名称</label>
      ...
      <div class="full">
        <label>后台管理令牌</label>
        <input type="text" name="token" id="tk" value="<?php echo ml_ins_h($V_TOKEN); ?>" placeholder="留空则自动生成一个 16 位随机令牌">
        <div class="hint">后台登录用；至少 6 位。<a href="javascript:void(0)" onclick="var s='abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789',o='';for(var i=0;i<16;i++){o+=s.charAt(Math.floor(Math.random()*s.length));}document.getElementById('tk').value=o;">点这里生成一个</a></div>
      </div>
      <div>
        <label>TMDB API Key（影视，可留空）</label>
        <input type="text" name="tmdb" value="<?php echo ml_ins_h($V_TMDB); ?>" placeholder="可稍后在后台填">
      </div>
      <div>
        <label>RAWG API Key（游戏，可留空）</label>
        <input type="text" name="rawg" value="<?php echo ml_ins_h($V_RAWG); ?>" placeholder="可稍后在后台填">
      </div>
      <div class="full">
        <label>启用哪些数据源</label>
        <div class="checks">
          <label><input type="checkbox" name="adapters[]" value="tmdb"<?php echo ml_ins_ck($V_ADS, 'tmdb'); ?>>TMDB（影视 / 剧集）</label>
          <label><input type="checkbox" name="adapters[]" value="rawg"<?php echo ml_ins_ck($V_ADS, 'rawg'); ?>>RAWG（游戏）</label>
          <label><input type="checkbox" name="adapters[]" value="bangumi"<?php echo ml_ins_ck($V_ADS, 'bangumi'); ?>>Bangumi（动漫，免 Key）</label>
        </div>
      </div>
    </div>
...
<?php endif; ?>
  ...
  <div class="foot">媒资资料库 · 安装向导<?php echo defined('ML_APP_VERSION') ? '' : ''; ?> ｜ 未安装状态专用页面</div>
</div>
</body>
</html>
