 ...
 *   GET  /user/forgot       找回密码（发邮件） POST 同址提交
 *   GET  /user/reset        邮件链接落地页     POST 设置新密码
 *   GET  /user/password     修改密码 / 绑邮箱  POST 同址提交（需登录）
...
if ($__seg === 'user/register') {
    if ($__isPost) {
        ...
        if (!$ok) {
            header('Location: /user/register' . ($em !== '' ? '?e=' . urlencode($em) : ''));
            exit;
        }
        /* v1.8.3 起账号一律用邮箱：邮箱即账号，不再填用户名/昵称 */
        $r = User::register(
            $em,
            ml_user_post('password'),
            ml_user_post('password2'),
            ml_user_post('captcha'),
            ml_user_post('ml_hp')            /* 蜜罐：真人看不见这个框 */
        );
...
/* 找回密码 /user/forgot                                             */
/* ================================================================ */
if ($__seg === 'user/forgot') {
    if ($__isPost) {
        $ok = ml_user_post_guard();
        if ($ok) {
            /* 防枚举：不管账号存不存在，成功与否都说同一句话 */
            User::requestReset(ml_user_post('account'), 'user');
            User::flashSet(
                'ok',
                '如果该账号存在且绑定了邮箱，重置链接已经发出，请在 60 分钟内查收邮件（也看看垃圾箱）。'
            );
        }
        header('Location: /user/forgot');
        exit;
    }
    echo UserViews::forgotHtml();
    return;
}

/* ================================================================ */
/* 重置密码 /user/reset?token=xxx                                    */
/* ================================================================ */
if ($__seg === 'user/reset') {
    ...
    if ($__isPost) {
        $ok = ml_user_post_guard();
        if ($ok) {
            $r = User::doReset($token, ml_user_post('password'), ml_user_post('password2'));
            User::flashSet($r['ok'] ? 'ok' : 'err', $r['msg']);
            if ($r['ok']) {
                header('Location: /user/login');
            } else {
                header('Location: /user/reset?token=' . urlencode($token));
            }
            exit;
        }
        header('Location: /user/reset?token=' . urlencode($token));
    ...
    echo UserViews::resetHtml($token, (User::verifyResetToken($token) !== null));
    return;
}

/* ================================================================ */
/* 修改密码 /user/password（需登录）                                  */
/* ================================================================ */
if ($__seg === 'user/password') {
    $uid = User::uid();
    if ($uid < 1) {
        User::flashSet('err', '请先登录再来修改密码。');
        header('Location: /user/login');
        exit;
    }
    if ($__isPost) {
        ...
        if ($ok) {
            ...
            if (array_key_exists('email', $_POST)) {
                $r = User::saveEmail($uid, ml_user_post('email'));
                $msg[] = $r['msg'];
                if (empty($r['ok'])) { $fail = true; }
            }
            /* 密码：三项里填了任意一项就按改密码处理 */
            $old = ml_user_post('old');
            $new = ml_user_post('new');
            if ($old !== '' || $new !== '' || ml_user_post('new2') !== '') {
                $r = User::changePassword($uid, $old, $new, ml_user_post('new2'));
                $msg[] = $r['msg'];
                if (empty($r['ok'])) { $fail = true; }
            }
            if (!$msg) { $msg[] = '没有需要保存的内容。'; }
            User::flashSet($fail ? 'err' : 'ok', implode('；', $msg));
        }
        header('Location: /user/password');
        exit;
    }
    echo UserViews::passwordHtml(User::me());
    return;
}
