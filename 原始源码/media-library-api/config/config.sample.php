 ...
 *
 * 说明：下面的「TMDB Key / RAWG Key / 启用数据源」以及网盘类型，
...
return [
    ...
    'adapters' => ['tmdb', 'rawg', 'hongguoduanju'],
    ...
    // 后台目录与显示（★ v1.7.0 起后台改为「账号 + 密码」登录，不再有 admin.token）
    //   管理员账号在安装时创建（app_users 表里第一个账号）；进后台用它登录即可。
    //   外部调用者的令牌是另一回事 —— 在后台「API 调用」页里逐个发放。
    'admin' => [
        'dir'    => 'admin',                // 后台目录名（必须以 admin 结尾；例：my-admin / secure-admin）
        'show_link' => 1,                   // 1=主页显示管理后台按钮；0=隐藏
        'show_icon' => 1,                   // 1=按钮带 🛡 图标；0=纯文字
    ],
    ...
    // 邮件服务（SMTP，v1.8.2 起）
    //   用于「找回密码 / 重置密码」发信。★ 建议在【后台 → 系统设置 → 邮件服务】里改，
    //   这里只是文件兜底值。QQ / 163 等邮箱的 pass 要填「授权码」，不是登录密码。
    'mail' => [
        'enable'    => 0,               // 1 = 启用邮件服务；0 = 停用（找回密码不可用）
        'host'      => '',              // SMTP 服务器，例：smtp.qq.com
        'port'      => 465,             // SSL 一般 465 / STARTTLS 一般 587 / 不加密 25
        'secure'    => 'ssl',           // ssl | tls（STARTTLS）| none
        'user'      => '',              // SMTP 账号（通常是完整邮箱地址）
        'pass'      => '',              // SMTP 密码 / 授权码
        'from'      => '',              // 发件人邮箱（必须与上面的账号一致）
        'from_name' => '',              // 发件人显示名；留空则用发件人邮箱
        'timeout'   => 15,              // 发信超时（秒）
    ],
];
