 ...
 *       v1.7.0 起改为「管理员登录态」判定：在后台登录过、或从本机/内网访问即可；
 ...
 * v1.7.0 起本函数仅在「尚未登录管理员」时兜底放行本机/内网访问。
 */
...
/* ★ v1.7.0：check.php 的访问门槛改成「管理员登录态」，不再用后台令牌。
   理由：后台登录早已改为账号 + 密码（见 admin/index.php），check.php 若还认令牌，
   就会出现「后台能进、自检进不去」的割裂。现在：已登录的管理员直接看；
   从服务器本机 / 内网访问也放行（方便在宝塔终端 curl 自检）。 */
$LOCAL_BYPASS = ml_is_local_request();

require_once __DIR__ . '/core/User.php';
$IS_ADMIN = class_exists('Core\\User') ? \Core\User::isAdmin() : false;

if (!$IS_ADMIN && !$LOCAL_BYPASS) {
    /* 一律以 404 收场：不出现「需要登录 / 权限不足」等字样，
       避免被扫描器确认本页存在。要看自检报告请先登录后台，再从「安全防护」页的一键入口点进来。 */
    ml_guard_deny();
}
...
/* ★ v1.7.0：后台改为账号 + 密码登录，这里改为检查「是否已创建管理员账号」 */
$adminCount = 0;
try {
    if (class_exists('Core\\DB')) {
        $st = \Core\DB::pdo()->query("SELECT COUNT(*) AS n FROM `app_users` WHERE `email`='ml-admin@local'");
        $r  = $st ? $st->fetch() : null;
        $adminCount = $r ? (int) $r['n'] : 0;
    }
} catch (\Exception $e) {}
$conf[] = ml_item('管理员账号', $adminCount > 0 ? 'ok' : 'fail',
    $adminCount > 0 ? '已创建 ' . $adminCount . ' 个管理员（用安装时那个账号登录后台）' : '未找到管理员账号，请重新安装或手工在 app_users 里建一个',
    $adminCount > 0 ? '' : '没有管理员账号就进不了后台');
...
$ovrKeys = ['tmdb_api_key' => 'TMDB key', 'rawg_api_key' => 'RAWG key', 'adapters' => '数据源', 'pan_types' => '网盘类型'];
$ovrOn   = [];
...
$srcLabel = ['tmdb' => 'TMDB（影视）', 'rawg' => 'RAWG（游戏）', 'hongguoduanju' => '红果短剧（国产短剧）'];
$srcName  = [];
...
/* v1.8.6：程序内置了会话目录兜底（core/Session.php），原目录不可写时会自动改用可写目录 */
$req11SpAlt = '';
if (!$req11SpOk && function_exists('ml_session_path')) {
    $__alt = ml_session_path();
    if ($__alt !== '' && $__alt !== $req11Sp) { $req11SpAlt = $__alt; }
}
$req11[] = ml_item('会话（登录状态）', ($req11SpOk || $req11SpAlt !== '') ? 'ok' : 'fail',
    '会话目录：' . ($req11Sp !== '' ? $req11Sp : '（未设置，用系统临时目录）') . ' · 自动开启 = ' . (ini_get('session.auto_start') ? '是' : '否')
    . ($req11SpAlt !== '' ? ' · 已自动改用可写目录：' . $req11SpAlt : ''),
    $req11SpOk ? '' : ($req11SpAlt !== ''
        ? '原会话目录不可写，程序已自动改用：' . $req11SpAlt . '（建议仍把原目录 chown 给 www 或改权限）'
        : '会话目录不可写 → 用户会「登录成功后又变回未登录」。改成可写目录或 chown 给 www 用户'));

/* 代码是否就位 */
$req11Files = ['core/User.php', 'core/UserViews.php', 'core/Session.php', 'user.php'];
$req11Lack  = [];
