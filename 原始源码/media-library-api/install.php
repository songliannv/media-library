...
$V_AUSER = ml_ins_val($FORM, 'admin_user', '');
...
<div class="wrap">
  ...
  <div class="hero">
    ...
    <p><?php echo $DONE
        ? '数据库已就绪、配置文件已生成、管理员账号已创建。可以直接登录后台了。'
        : '跟着下面两步走：环境自检 → 填数据库与管理员账号 → 点一下安装。'; ?></p>
  </div>
...
location ~* ^/(setup|upgrade|update) { return 404; }</pre>
      ...
      <li style="margin:8px 0"><b>2. 登录后台并配置</b>：<code><?php echo ml_ins_h($BASE_URL); ?>/admin</code> —— 用上面的管理员账号登录<br>
        进去后到「<b>系统设置</b>」填 TMDB / RAWG Key；到「<b>站点设置</b>」填站点名称、网址、SEO。</li>
