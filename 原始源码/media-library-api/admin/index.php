 ...
 *   ① 本文件本身就是一个合法入口（后台登录页），不能再被 Guard 当成「内部文件」404。
 *      宝塔 Nginx 常把 /admin/ 直接落到本文件（不经根 index.php），此时 ML_APP 未定义，
 *      旧代码会在 ml_guard_shield(__FILE__) 里把自己 404 掉。这里先补上定义。
 *   ② 会话统一走 core/Session.php：会话目录不可写时自动换目录，反代下正确识别 HTTPS。
 *   ③ 登录蜜罐字段由 website 改为 ml_hp —— website 会被浏览器 / 密码管理器自动填充，
 *      导致「检测到异常提交，已忽略」，表现为登录无反应。
 */

use Core\Config;
use Core\User;

if (!defined('ML_APP'))         { define('ML_APP', true); }
if (!defined('ML_ROOT'))        { define('ML_ROOT', dirname(__DIR__)); }
if (!defined('ML_APP_VERSION')) { define('ML_APP_VERSION', '1.8.6'); }
...
/* 后台 CSRF 令牌。会话名与前台统一（MLSESSID），并做「目录不可写」兜底。 */
$SESSION_OK = ml_session_boot(User::SESS);
function ml_ad_csrf()
{
    if (empty($_SESSION['ml_ad_csrf'])) {
        if (function_exists('openssl_random_pseudo_bytes')) {
            $_SESSION['ml_ad_csrf'] = bin2hex(openssl_random_pseudo_bytes(16));
        } else {
            $_SESSION['ml_ad_csrf'] = md5(uniqid((string) mt_rand(), true));
        }
    }
    return (string) $_SESSION['ml_ad_csrf'];
}

$ADMIN_DIR = trim((string) Config::sub('admin', 'dir', 'admin'));
if ($ADMIN_DIR === '' || strpos($ADMIN_DIR, 'admin') !== strlen($ADMIN_DIR) - 5) { $ADMIN_DIR = 'admin'; }
$ADMIN_URL = '/' . $ADMIN_DIR . '/';
...
/* ---- 登录提交 ---- */
$__adDo = isset($_POST['ad_do']) ? (string) $_POST['ad_do'] : '';
/* 兜底：万一隐藏字段被代理 / 密码管理器吃掉，只要带着账号密码就仍按登录处理 */
if ($__adDo === '' && isset($_POST['u'], $_POST['p'])) { $__adDo = 'login'; }

if ($__adDo === 'login') {
    $FOCUS = 'user';
    if (!$SESSION_OK) {
        $LOGIN_ERR = '服务器无法保存登录会话（session 不可写），所以登不上去。'
                   . '请打开 check.php 看「会话」一栏，或让服务商把 PHP 的 session.save_path 目录设为可写。';
    } elseif (!isset($_POST['csrf']) || !hash_equals(ml_ad_csrf(), (string) $_POST['csrf'])) {
        $LOGIN_ERR = '页面已过期（登录会话没保持住），请再提交一次；若反复如此，请打开 check.php 看「会话」一栏。';
    } elseif (isset($_POST['ml_hp']) && trim((string) $_POST['ml_hp']) !== '') {
        $LOGIN_ERR = '检测到异常提交，已忽略。';
    } else {
        $u = isset($_POST['u']) ? (string) $_POST['u'] : '';
        ...
        if (!empty($r['ok'])) {
            ...
            User::logout();
            $LOGIN_ERR = '该账号不是管理员，无法进入后台。';
        } else {
            $LOGIN_ERR = (string) $r['msg'];
        }
    }
}

/* ---- 找回密码提交（未登录即可用）：发一封带重置链接的邮件 ---- */
if (isset($_POST['ad_do']) && $_POST['ad_do'] === 'forgot') {
    if (!isset($_POST['csrf']) || !hash_equals(ml_ad_csrf(), (string) $_POST['csrf'])) {
        $LOGIN_ERR = '页面已过期，请重新提交。';
    } else {
        $r = User::requestReset(isset($_POST['account']) ? (string) $_POST['account'] : '', 'admin');
        if (!empty($r['ok'])) { $LOGIN_OK  = (string) $r['msg']; }
        else                  { $LOGIN_ERR = (string) $r['msg']; }
    }
}

/* ---- 重置密码提交（从邮件链接点进来）：用一次性令牌设置新密码 ---- */
if (isset($_POST['ad_do']) && $_POST['ad_do'] === 'reset') {
    if (!isset($_POST['csrf']) || !hash_equals(ml_ad_csrf(), (string) $_POST['csrf'])) {
        $LOGIN_ERR = '页面已过期，请重新提交。';
    } else {
        $r = User::doReset(
            isset($_POST['token']) ? (string) $_POST['token'] : '',
            isset($_POST['new'])   ? (string) $_POST['new']   : '',
            isset($_POST['new2'])  ? (string) $_POST['new2']  : ''
        );
        if (!empty($r['ok'])) { $LOGIN_OK  = (string) $r['msg'] . ' 请用新密码登录。'; }
        else                  { $LOGIN_ERR = (string) $r['msg']; }
    }
}
...
/* ---- 登录闸门：未登录 or 非管理员 → 只输出登录页 ---- */
$ME = User::me();
if (!$ME || !User::isAdmin()) {
    /* 未登录时的三种视图：login（登录）/ forgot（找回）/ reset（设置新密码） */
    $MODE = 'login';
    $RESET_TOKEN = '';
    if (isset($_GET['reset'])) { $MODE = 'reset'; $RESET_TOKEN = (string) $_GET['reset']; }
    elseif (isset($_GET['forgot'])) { $MODE = 'forgot'; }
    /* 刚提交重置成功 → 回到登录视图显示成功提示 */
    if ($LOGIN_OK !== '' && $MODE === 'reset') { $MODE = 'login'; }
    $resetValid = ($MODE === 'reset') ? (User::verifyResetToken($RESET_TOKEN) !== null) : false;
?><!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
...
<div class="box">
  <div class="head">
    <div class="logo">🛡</div>
    <h1>媒资资料库 · 管理后台</h1>
    <p><?php echo ($MODE === 'forgot') ? '找回密码' : (($MODE === 'reset') ? '设置新密码' : '请用管理员账号登录'); ?></p>
  </div>
  <div class="card">
    <?php if (!$SESSION_OK): ?>
    ...
    <?php endif; ?>
    <?php if ($LOGIN_ERR !== ''): ?>
    <div class="err"><?php echo htmlspecialchars($LOGIN_ERR, ENT_QUOTES, 'UTF-8'); ?></div>
    <?php endif; ?>
    <?php if ($LOGIN_OK !== ''): ?>
    <div class="ok"><?php echo htmlspecialchars($LOGIN_OK, ENT_QUOTES, 'UTF-8'); ?></div>
    <?php endif; ?>

    <?php if ($MODE === 'forgot'): ?>
    <!-- 找回密码：输入账号 / 邮箱，发一封带重置链接的邮件 -->
    <form method="post" action="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>?forgot=1" autocomplete="off">
    ...
    <p class="mini">重置链接会发到账号绑定的邮箱，1 小时内有效、只能用一次。<br>
      ...
      <a href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>">← 返回登录</a></p>

    <?php elseif ($MODE === 'reset'): ?>
    <?php if (!$resetValid): ?>
    <!-- 链接无效 / 已过期 -->
    <div class="err">这个重置链接无效或已过期（可能已被使用，或超过 1 小时）。请重新发起「找回密码」。</div>
    <p class="mini"><a href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>?forgot=1">重新找回密码</a>　·　<a href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>">返回登录</a></p>
    <?php else: ?>
    <!-- 设置新密码 -->
    <form method="post" action="<?php echo htmlspecialchars($ADMIN_URL . '?reset=' . urlencode($RESET_TOKEN), ENT_QUOTES, 'UTF-8'); ?>" autocomplete="off">
    ...
    <p class="mini"><a href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>">← 返回登录</a></p>
    <?php endif; ?>

    <?php else: ?>
    <!-- 正常登录 -->
    <form method="post" action="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>" autocomplete="off">
    ...
    <p class="mini"><a href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>?forgot=1">忘记密码？</a></p>
    <?php endif; ?>
  </div>
  <div class="foot">
    这不是给外部调用者的入口。<br>
...
<div class="wrap">
  <div class="token-bar">
    ...
    <b style="margin-right:auto"><?php echo htmlspecialchars((string) $ME['username'], ENT_QUOTES, 'UTF-8'); ?></b>
    <button type="button" class="ghost sm" onclick="openPwd()">修改密码</button>
    <a class="ghost" href="<?php echo htmlspecialchars($ADMIN_URL, ENT_QUOTES, 'UTF-8'); ?>?logout=1"
...
id="tab-users" class="tabpane" style="display:none">
    <div class="card">
      <h2>用户中心</h2>
      <div class="toolbar">
...
<div class="modal" id="pwdModal">
  <div class="box" style="max-width:470px">
    ...
    <?php
      $__myEmail = (string) $ME['email'];
      if ($__myEmail === \Core\User::ADMIN_TAG) { $__myEmail = ''; }
    ?>
    <label>账号邮箱（找回密码用，可留空）</label>
    <input id="pw_email" type="email" value="<?php echo htmlspecialchars($__myEmail, ENT_QUOTES, 'UTF-8'); ?>" placeholder="you@example.com">
...
let SET_TS    = {};    // 数据源连通状态：{tmdb:{ok,label,detail}, rawg:{…}, hongguoduanju:{…}}
const SRCS = [
  {kind:'tmdb',    name:'TMDB'},
  {kind:'rawg',    name:'RAWG'},
...
const TXT_KEYS = ['tmdb_api_key','rawg_api_key'];
const AD_KEYS  = ['tmdb','rawg','hongguoduanju'];
...
function renderSettings(){
  ...
  /* 数据源连通状态栏：一次把 TMDB / RAWG / 红果短剧 三个源试一遍 */
  h += `<div class="status-bar">
    <span class="pill">TMDB：<b id="st_tmdb">${SET_TS.tmdb?SET_TS.tmdb.label:'未测试'}</b></span>
    <span class="pill">RAWG：<b id="st_rawg">${SET_TS.rawg?SET_TS.rawg.label:'未测试'}</b></span>
    <span class="pill">红果短剧：<b id="st_hongguoduanju">${SET_TS.hongguoduanju?SET_TS.hongguoduanju.label:'未测试'}</b></span>
  ...
  for(const k in opts){
    ...
    const kind = (k==='tmdb') ? 'tmdb' : (k==='rawg' ? 'rawg' : (k==='hongguoduanju' ? 'hongguoduanju' : ''));
 ...
 * 之所以要区分，是因为以前无论点哪里都会去读输入框；TMDB/RAWG 的输入框
 * 初始值就是已保存的值，看似能用；而 红果短剧 从来没有输入框，于是压根没按钮。
 */
async function testKey(kind, useDraft){
  const id = (kind==='tmdb') ? 's_tmdb_api_key' : (kind==='rawg' ? 's_rawg_api_key' : '');
...
async function testAllSources(){
  ...
  const msg = document.getElementById('setMsg');
  if(msg){ msg.style.color='var(--mut)'; msg.innerText='正在依次测试 TMDB / RAWG / 红果短剧 …（连不上时每个源最多等 10 秒）'; }
  let ok=0, fail=0;
  await busy(btn, '测试中…', async ()=>{
    for(const s of SRCS){
