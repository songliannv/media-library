...
use Adapters\Tmdb;
use Adapters\Rawg;
use Adapters\HongguoDuanju;

/**
 * 统一 API + 后台管理端点
 *
 * 公开（v1.7.0 起可选用调用令牌；带令牌则记录用量，不带则按匿名调用）：
 ...
 *
 *   调用令牌（给外部调用者，在后台「API 调用」页发放）：
 *     - 请求头  X-Api-Token: <token>
 *     - 或参数  ?token=<token>
 *   每个令牌可单独设置每日调用上限与 IP 白名单，调用流水记入 app_api_logs。
 *
 * 后台（需「管理员账号登录态」，仅站长自己用）：
 ...
 *   GET  /api/v1/admin/token_list|token_stats   （API 调用令牌：列表 / 概览数字）
 *   POST /api/v1/admin/token_add|token_save|token_del （API 调用令牌：新建 / 修改 / 删除）
 *   GET  /api/v1/admin/api_logs?token_id=&start=&end= （API 调用流水）
 */
class Unified
{
    public static function handle($path)
    {
        ...
        // 验证公开 API 的调用令牌（v1.7.0：给外部调用者发独立令牌）
        $apiToken = null;
        $tk = '';
        if (isset($_SERVER['HTTP_X_API_TOKEN'])) { $tk = (string) $_SERVER['HTTP_X_API_TOKEN']; }
        elseif (isset($q['token'])) { $tk = (string) $q['token']; }
        if ($tk !== '') {
            if (!class_exists('Core\\ApiToken')) { require_once __DIR__ . '/../core/ApiToken.php'; }
            $apiToken = \Core\ApiToken::verify($tk);
            if ($apiToken !== null) {
                $__t0 = microtime(true);
                register_shutdown_function(function () use ($apiToken, $rest, $q, $__t0) {
                    \Core\ApiToken::log(
                        $apiToken['id'], $apiToken['name'], '/' . $rest,
                        isset($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : 'GET',
                        $q, 200, (int) ((microtime(true) - $__t0) * 1000)
                    );
                });
            }
        }
    ...
    private static function admin($rest)
    {
        ...
        if ($action === 'item') {
            ...
            $body = self::query();
            if ($method === 'POST' && $id === 0) {
                // ★ v1.8.9: 自动下载海报和背景图到本地
                $__downloadImages = isset($_POST['download_images']) && $_POST['download_images'] == '1';
                $__uploadDir = __DIR__ . '/../uploads/';
                if ($__downloadImages && !is_dir($__uploadDir)) { @mkdir($__uploadDir, 0755, true); }
                if ($__downloadImages && function_exists('file_get_contents') && function_exists('file_put_contents')) {
                    foreach (['poster', 'backdrop'] as $__imgType) {
                        if (isset($_POST[$__imgType]) && filter_var($_POST[$__imgType], FILTER_VALIDATE_URL)) {
                            $__imgUrl = $_POST[$__imgType];
                            $__imgExt = pathinfo(parse_url($__imgUrl, PHP_URL_PATH), PATHINFO_EXTENSION);
                            if (!in_array(strtolower($__imgExt), ['jpg', 'jpeg', 'png', 'webp', 'gif'])) { $__imgExt = 'jpg'; }
                            $__imgName = md5($__imgUrl) . '.' . $__imgExt;
                            $__imgPath = $__uploadDir . $__imgName;
                            if (!file_exists($__imgPath)) {
                                $__imgContent = @file_get_contents($__imgUrl);
                                if ($__imgContent !== false) { @file_put_contents($__imgPath, $__imgContent); }
                            }
                            $_POST[$__imgType] = '/uploads/' . $__imgName;
                        }
                    }
                }
        ...
        if ($action === 'settings_reset' && $method === 'POST') {
            ...
            $view = self::settingsView();
            Json::ok(['reset' => true, 'items' => $view['items']]);
        }

        // 测试上游 Key 是否可用（TMDB / RAWG）
        if ($action === 'settings_test' && $method === 'POST') {
            try {
                ...
                Json::error(400, '探测失败：' . $e->getMessage());
            } catch (\Error $e) {
                Json::error(500, '探测失败：' . $e->getMessage());
            }
        }

        // 发送一封 SMTP 测试邮件（验证「站点设置 → 邮件服务」配置是否正确；
        // 支持传还没保存的草稿值，先测再存）
        if ($action === 'mail_test' && $method === 'POST') {
            try {
                require_once __DIR__ . '/../core/Mail.php';
                $body = self::query();
                $to   = trim((string) ((isset($body['to']) ? $body['to'] : '')));
                if ($to === '') {
                    $to = trim((string) ((isset($body['mail_from']) ? $body['mail_from'] : '')));
                }
                if ($to === '') { $to = trim((string) Config::sub('mail', 'from', '')); }
                if ($to === '') {
                    Json::error(400, '请先填好「发件人邮箱」，或在收件人里指定一个地址再测试。');
                }
                /* 传了哪个 mail_* 键就覆盖哪个（草稿测试） */
                $ovKeys = array('enable', 'host', 'port', 'secure', 'user', 'pass', 'from', 'from_name');
                $ov = array();
                foreach ($ovKeys as $mk) {
                    $bk = 'mail_' . $mk;
                    if (array_key_exists($bk, $body)) { $ov[$mk] = $body[$bk]; }
                }
                /* 结果里带 ok / msg / log，成功失败都返回 200，前端按 data.ok 展示 */
                $r = \Core\Mail::test($to, $ov);
                $r['log'] = \Core\Mail::lastLog();
                Json::ok($r);
            } catch (\Exception $e) {
                Json::error(400, '测试失败：' . $e->getMessage());
            } catch (\Error $e) {
                Json::error(500, '测试失败：' . $e->getMessage());
            }
        }

        // 修改当前登录账号的密码 / 邮箱（后台「修改密码」）
        if ($action === 'password' && $method === 'POST') {
            try {
                require_once __DIR__ . '/../core/User.php';
                $uid = \Core\User::uid();
                if ($uid < 1) { Json::error(401, '登录状态已失效，请重新登录。'); }

                $body  = self::query();
                $msgs  = array();
                $email = array_key_exists('email', $body) ? trim((string) $body['email']) : null;
                $old   = (string) ((isset($body['old'])  ? $body['old']  : ''));
                $new   = (string) ((isset($body['new'])  ? $body['new']  : ''));
                $new2  = (string) ((isset($body['new2']) ? $body['new2'] : ''));

                /* 邮箱：传了就保存（留空 = 清除绑定） */
                if ($email !== null) {
                    $re = \Core\User::saveEmail($uid, $email);
                    if (empty($re['ok'])) { Json::error(400, (string) $re['msg']); }
                    $msgs[] = '邮箱已保存';
                }

                /* 密码：只要填了任一项就按「改密码」处理 */
                if ($old !== '' || $new !== '' || $new2 !== '') {
                    $r = \Core\User::changePassword($uid, $old, $new, $new2);
                    if (empty($r['ok'])) { Json::error(400, (string) $r['msg']); }
                    $msgs[] = (string) $r['msg'];
                }

                Json::ok(array('msg' => $msgs ? implode('；', $msgs) : '没有需要保存的内容。'));
            } catch (\Exception $e) {
                Json::error(400, '修改失败：' . $e->getMessage());
            } catch (\Error $e) {
                Json::error(500, '修改失败：' . $e->getMessage());
            }
        }
        ...
        /* ---- API 令牌管理：给外部调用者发独立令牌（v1.7.0） ---- */
        if ($action === 'token_list' && $method === 'GET') {
            require_once __DIR__ . '/../core/ApiToken.php';
            $q    = self::query();
            $page = (int) ((isset($q['page']) ? $q['page'] : 1));
            $out  = \Core\ApiToken::listTokens($page);
            // 附带今日用量，列表页直接显示「今日/限额」
            $today = \Core\ApiToken::todayUsageMap();
            foreach ($out['items'] as $i => $row) {
                $out['items'][$i]['today_calls'] = isset($today[(int) $row['id']]) ? $today[(int) $row['id']] : 0;
            }
            Json::ok($out);
        }

        if ($action === 'token_stats' && $method === 'GET') {
            require_once __DIR__ . '/../core/ApiToken.php';
            Json::ok(\Core\ApiToken::getStats());
        }

        if ($action === 'token_add' && $method === 'POST') {
            require_once __DIR__ . '/../core/ApiToken.php';
            $b     = self::query();
            $name  = trim((string) ((isset($b['name']) ? $b['name'] : '')));
            if ($name === '') { Json::error(400, '请填写令牌名称'); }
            $token = \Core\ApiToken::create(
                $name,
                (int) ((isset($b['user_id']) ? $b['user_id'] : 0)),
                (int) ((isset($b['daily_limit']) ? $b['daily_limit'] : 0)),
                (string) ((isset($b['ip_whitelist']) ? $b['ip_whitelist'] : ''))
            );
            if ($token === null) { Json::error(500, '创建失败，请检查数据库'); }
            Json::ok(array('token' => $token));
        }

        if ($action === 'token_save' && $method === 'POST') {
            require_once __DIR__ . '/../core/ApiToken.php';
            $b  = self::query();
            $id = (int) ((isset($b['id']) ? $b['id'] : 0));
            if ($id < 1) { Json::error(400, '缺少记录 id'); }
            $ok = \Core\ApiToken::update($id, $b);
            Json::ok(array('id' => $id, 'saved' => $ok));
        }
        ...
        if ($action === 'token_del' && $method === 'POST') {
            ...
            $id = (int) ((isset($b['id']) ? $b['id'] : 0));
            if ($id < 1) { Json::error(400, '缺少记录 id'); }
            Json::ok(array('id' => $id, 'deleted' => \Core\ApiToken::delete($id)));
        }

        if ($action === 'api_logs' && $method === 'GET') {
            require_once __DIR__ . '/../core/ApiToken.php';
            $q     = self::query();
            $page  = (int) ((isset($q['page']) ? $q['page'] : 1));
            $tid   = (int) ((isset($q['token_id']) ? $q['token_id'] : 0));
            $start = (string) ((isset($q['start']) ? $q['start'] : ''));
            $end   = (string) ((isset($q['end']) ? $q['end'] : ''));
            Json::ok(\Core\ApiToken::getLogs($tid > 0 ? $tid : null, $start !== '' ? $start : null, $end !== '' ? $end : null, $page));
        }
    ...
    private static function settingsView()
    {
        ...
        // 两个文本项：与 config.php 的二级键对应
        // ★ v1.7.0：admin_token 已移除（后台改用账号 + 密码登录）
        $map = array(
        ...
        // 邮件服务（SMTP）：用于「找回密码 / 重置密码」发信
        foreach (\Core\Settings::mailMap() as $k => $child) {
            $db   = (string) \Core\Settings::get($k, '');
            $file = Config::baseSub('mail', $child, null);
            $file = ($file === null) ? '' : (string) $file;
            $items[$k] = array(
                'value'    => $db !== '' ? $db : $file,
                'db'       => $db,
        ...
        /* ★ v1.8.7：站点设置各组（site / seo / redirect / pan / pansou / req …）
           以前从不构建 items，前端 fieldRow()/列表勾选拿不到「当前值」，
           于是文本框永远空白、勾选框永远不显示已选；保存时还会把没碰过的
           字段一起写成空值（= 清掉覆盖）。这里按 groupMaps 统一补齐。
           sync / mail 已在上方单独处理（带 default 兜底），跳过不要覆盖。 */
        $schema = \Core\Settings::schema();
        foreach (\Core\Settings::groupMaps() as $g => $gmap) {
            foreach ($gmap as $k => $child) {
                if (isset($items[$k])) { continue; }
                $db   = (string) \Core\Settings::get($k, '');
                $file = Config::baseSub($g, $child, '');
                $file = ($file === null) ? '' : (string) $file;
                if (isset($schema[$k]['type']) && $schema[$k]['type'] === 'list') {
                    $dbA   = \Core\Settings::strToList($db);
                    $fileA = \Core\Settings::strToList($file);
                    $items[$k] = array(
                        'value'    => $dbA ? $dbA : $fileA,
                        'db'       => $dbA,
                        'file'     => $fileA,
    ...
    private static function saveSettings(array $body)
    {
        ...
        // ★ v1.7.0：不再处理 admin_token（后台改为账号 + 密码登录）
        ...
        if (array_key_exists('adapters', $body)) {
            ...
            $allow = array('tmdb', 'rawg', 'hongguoduanju');
            $pick = array();
            foreach ($list as $x) {
        ...
        $ranges = array(
            ...
            /* 站点设置 · 邮件服务 */
            'mail_port'         => array(1, 65535, 'SMTP 端口'),
            'mail_timeout'      => array(5, 60, '发信超时(秒)'),
        );
        $enums = array(
            ...
            /* 站点设置 · 邮件服务 */
            'mail_enable'          => array(array('0', '1'), '是否启用邮件服务'),
            'mail_secure'          => array(array('ssl', 'tls', 'none'), 'SMTP 加密方式'),
        );
        ...
        foreach ($flat as $k => $child) {
            ...
            /* ★ v1.8.7：列表型设置（nav_show / pan_group / pansou_types …）
               前端提交的是数组，必须先转成逗号串再落库；否则会被下面的
               is_array()?'':… 判成空值，set() 里 '' 又代表「清除覆盖」，
               结果就是「勾了保存却等于没勾」。 */
            if (isset($schema[$k]['type']) && $schema[$k]['type'] === 'list') {
                $set[$k] = \Core\Settings::listToStr($body[$k]);
                continue;
            }
    ...
    /**
     * 轻量探测上游数据源是否可用（用 LinkChecker，HEAD 失败即回退 GET）。
     *  - tmdb / rawg：带 Key 拜访一个真实接口，HTTP 200 即视为 Key 可用；
     *  - hongguoduanju：需要配置Base URL「每日放送」接口 /calendar（根路径 / 会返回 404，
     *    红果短剧适配器说明"异常"）。
     * @param string $kind        tmdb | rawg | hongguoduanju
     * @param string $keyOverride 用界面上正在编辑（还没保存）的 Key 来测；空串 = 用已保存的值
     */
    private static function testUpstream($kind, $keyOverride = '')
    {
        $urls = array(
            ...
            'hongguoduanju' => 'https://api.bgm.tv/calendar',
        );
        if (!isset($urls[$kind])) {
            return array('ok' => false, 'code' => 0, 'msg' => '未知的数据源');
        }

        $name = ($kind === 'tmdb') ? 'TMDB' : (($kind === 'rawg') ? 'RAWG' : '红果短剧');

        if ($kind === 'hongguoduanju') {
            // 免 Key 数据源：无需配置即可直接探测
            $url = $urls[$kind];
        } else {
            $key = trim($keyOverride);
            if ($key === '') {
                $key = (string) Config::sub($kind, 'api_key', '');
            }
            if ($key === '') {
                return array('ok' => false, 'code' => 0, 'msg' => '尚未填写 ' . $name . ' API Key —— 填好再点「测试此 Key」');
            }
        ...
        $msg = '';
        if (!$r['ok']) {
            if ((int) $r['code'] === 401 || (int) $r['code'] === 403) {
                $msg = ($kind === 'hongguoduanju')
                    ? '上游返回 ' . (int) $r['code'] . '（多为请求过于频繁或被限流，稍后再试）'
                    : 'Key 无效或被拒绝（HTTP ' . (int) $r['code'] . '）—— 请到上游后台确认 Key 是否正确、是否已启用';
            } elseif ((int) $r['code'] === 0) {
                $msg = '连不上该站点：' . $r['msg'] . '（常见于服务器在境内且未放行境外请求，可让服务商开放 api.' . ($kind === 'tmdb' ? 'themoviedb.org' : ($kind === 'rawg' ? 'rawg.io' : 'bgm.tv')) . '）';
            } elseif ((int) $r['code'] === 404 || (int) $r['code'] === 400) {
                $msg = '上游返回 HTTP ' . (int) $r['code'] . '（接口地址或参数有误）';
            } else {
                $msg = 'HTTP ' . (int) $r['code'];
            }
        }
    ...
    /**
     * 后台接口鉴权（v1.7.0 起改用「账号 + 密码」登录态，不再用管理令牌）
     *
     * 为什么改：原先后台靠一个全局 admin_token，相当于「自己给自己发密钥才能管自己」，
     * 毫无意义还把外部调用者的密钥与站长的登录凭据混在一起。现在：
     *   · 后台登录 = 管理员账号（安装时创建的第一个账号），态存 $_SESSION；
     *   · 外部调用者 = 「API 调用」页发的独立令牌（X-Api-Token / ?token=），只作用于公开接口。
     */
    private static function requireAdmin()
    {
        require_once __DIR__ . '/../core/User.php';
        if (!class_exists('Core\\User')) { Json::error(500, 'server error'); }
        if (!\Core\User::isAdmin()) { Json::error(401, 'unauthorized'); }
    }
