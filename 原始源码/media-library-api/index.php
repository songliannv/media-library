...
// ★ v1.6.5：后台目录来自 config.admin.dir，默认 admin，必须末尾含 admin
$__admDir = trim((string) Config::sub('admin', 'dir', 'admin'));
if ($__admDir === '') { $__admDir = 'admin'; }
if (strpos($__admDir, 'admin') !== strlen($__admDir) - 5) { $__admDir = 'admin'; }
$__admPrefix = '/' . ltrim($__admDir, '/');
if (strpos($path, $__admPrefix) === 0) {
    require __DIR__ . '/' . $__admDir . '/index.php';
    return;
}
if ($path === '/' || $path === '') {
    /* ------------------------------------------------------------ */
